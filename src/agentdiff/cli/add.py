...
def _resolve_change(args: argparse.Namespace, store: Store) -> Change:
...
def run(args: argparse.Namespace, store: Store, out: TextIO) -> int:
    change = _resolve_change(args, store)
    ...
    comment = Comment(
        ...
        updated_at=now,
        anchor_snapshot=(
            snapshot_lines(current, args.file, line_range) if line_range else []
        ),
    )
