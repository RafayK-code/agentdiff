...
def test_markdown_header(approved_change, mixed_comments: list[object]) -> None:
    out = export_markdown(approved_change, mixed_comments)
    assert out.startswith("# Change chg-01\n\n")


def test_markdown_per_file_sections(
...
def test_markdown_section_even_without_comments() -> None:
...
def test_markdown_bullet_format() -> None:
...
def test_markdown_location_old() -> None:
...
def test_markdown_location_file_level() -> None:
    change = Change(id="chg-x", files=[FileDiff(path="a.txt")])
...
def test_markdown_state_strings() -> None:
    change = Change(id="chg-x", files=[FileDiff(path="a.txt")])
    comments = [
...
def test_markdown_trailing_newline(
    approved_change, mixed_comments: list[object]
) -> None:
    out = export_markdown(approved_change, mixed_comments)
    assert out.endswith("\n")
...
def test_markdown_file_order() -> None:
    change = Change(
        id="chg-x",
...
def test_markdown_comment_input_order() -> None:
    change = Change(id="chg-x", files=[FileDiff(path="a.txt")])
...
def test_markdown_orphan_file_fallback() -> None:
    change = Change(id="chg-x", files=[FileDiff(path="a.txt")])
